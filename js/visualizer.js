...
var canvas = $('canvas');
var MP3_PATH = '';
var sketch, analyser, ALPHA, AudioAnalyser, COLORS, NUM_BANDS, NUM_PARTICLES, Particle, SCALE, SIZE, SMOOTHING, SPEED, SPIN;

$('.section.listen ul li').on('click', function (evt) {

    if(analyser){
        analyser.audio.remove();
        sketch.destroy();
    }

    MP3_PATH = $(evt.target).attr('data-src');
    window.visualize();
});
...
window.visualize = function () {
    ...
    $('.section.listen').prepend(sketch.element)
